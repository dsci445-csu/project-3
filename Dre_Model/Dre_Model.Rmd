...
train_data <- fread("final_train_lags.csv")
...
```{r}
# Step 3: Define Columns and Check Missingness
...
# Check missingness in training and test datasets
...
# Step 4: Induce Missingness for Validation
set.seed(123)
train_data[sample(.N, size = 0.1 * .N), sales_lag_7 := NA]

# Step 5: Split Data for Cross-Validation
set.seed(123)
train_split <- train_data[1:(0.8 * .N)]
valid_split <- train_data[(0.8 * .N + 1):.N]

# Convert `transactions` to double for consistent handling
...
# Predict on Validation Set (Native Handling)
...
# Recreate DMatrix for Rolling Average Imputed Model
...
# Predict on Validation Set (Rolling Average)
...
# Optionally print a sample of the dumped file
