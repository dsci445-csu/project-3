...
ts_data <- ts(
  time_series_data$total_sales,
  frequency = 7,
  start = c(year(min(time_series_data$date)), yday(min(time_series_data$date)))
)

sarima_model <- Arima(ts_data, order = c(5,1,1), seasonal = c(2, 1, 2))
...
```{r}
holiday <- read.csv("../Data/holidays_events.csv")

refined_model <- Arima(ts_data, 
                       xreg = cbind(sales_oil_data$oil_price),
                       order = c(2, 1, 2), 
                       seasonal = c(3, 1, 2))

checkresiduals(refined_model)
