...
ggplot(ols_metrics_long, aes(x = dataset, y = Value, fill = Metric)) +
  ...
  geom_text(aes(label = round(Value, digits = 3)), color ="black", vjust = 1.5,
...
ohtani <- data_2025 |> filter(WAR == 6.6 & PA == 727)
...
ohtani_preds <- augment(ols_fit, ohtani) 
ohtani_preds <- ohtani_preds |> select(c(".pred", "WAR"))

example_preds <- data.frame("Player" = c("Aaron Judge", "Hunter Goodman", "Michael Toglia", "Bobby Witt Jr.", "Shohei Ohtani"),
                            "Prediction" = round(c(judge_preds$.pred, goodman_preds$.pred, toglia_preds$.pred, witt_preds$.pred,ohtani_preds$.pred ), digits = 1),
                            "Actual" = c(judge_preds$WAR, goodman_preds$WAR, toglia_preds$WAR, witt_preds$WAR, ohtani_preds$WAR))
