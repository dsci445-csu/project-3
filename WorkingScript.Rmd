...
ols_fit <- ols_spec |>
  fit(WAR ~ ., data = data_2025)

summary(ols_fit$fit)

set.seed(445)
data_split <- initial_split(data_2025, prop = 0.8)
train <- training(data_split)
test <- testing(data_split)
...
lasso_recipe <- recipe(WAR ~ ., data = data_2025)
...
lasso_final_fit <- lasso_final |> fit(data_2025)

lasso_preds_train <- lasso_final_fit |> augment(data_2025)
...
boost_folds <- vfold_cv(data_2025, v = 10)

boost_recipe <- recipe(WAR ~ ., data = data_2025)
...
boost_final_fit <- boost_final |> fit(data = data_2025)

boost_preds <- augment(boost_final_fit, data_2025)

boost_rsme <- rsme(boost_preds, truth = WAR, estimate =.pred)$.estimate
boost_mae <- mae(boost_preds, truth = WAR, estimate =.pred)$.estimate
