...
data_2025 <- data_2025 |> select(-c("Awards", "Pos", "Team", "Lg", "Player"))
...
ols_rmse_train <- rmse(ols_preds_train, truth = WAR, estimate = .pred)$.estimate
ols_mae_train  <- mae(ols_preds_train,  truth = WAR, estimate = .pred)$.estimate

ols_rmse_test <- rmse(ols_preds_test, truth = WAR, estimate = .pred)$.estimate
ols_mae_test  <- mae(ols_preds_test,  truth = WAR, estimate = .pred)$.estimate
...
boost_spec <- boost_tree(
  ...
  tree_depth     = tune(),
  learn_rate     = tune(),
  min_n          = tune(), #min number of decision trees
  loss_reduction = 0,
  sample_size    = 1
) |>
  set_mode("regression") |>
...
#OLS
preds <- predict(ols_fit, data_2025) |> bind_cols(data_2025)

#LASSO
lasso_preds_test <- lasso_final_fit |> augment(data_2025)

#BOOSTING
boost_preds_test <- augment(boost_final_fit, data_2025)
...
# OLS
ols_rmse_2025 <- rmse(ols_preds_2025, truth = WAR, estimate = .pred)$.estimate
ols_mae_2025  <- mae(ols_preds_2025,  truth = WAR, estimate = .pred)$.estimate

# LASSO
lasso_rmse_2025 <- rmse(lasso_preds_2025, truth = WAR, estimate = .pred)$.estimate
lasso_mae_2025  <- mae(lasso_preds_2025,  truth = WAR, estimate = .pred)$.estimate

# BOOSTING
boost_rmse_2025 <- rmse(boost_preds_2025, truth = WAR, estimate = .pred)$.estimate
boost_mae_2025  <- mae(boost_preds_2025,  truth = WAR, estimate = .pred)$.estimate
