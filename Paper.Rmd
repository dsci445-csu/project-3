...
author: "Ari Crumley, Vincent West, Gioia Bonanno-Garcia"
date: "2025-12-05"
output:
...
knitr::opts_knit$set(
  `header-includes` = c("\\usepackage{float}")
)
```
...
## Introduction
For our project, we wanted to build a model that could accurately predict the popular baseball performance metric, Wins Above Replacement (WAR). WAR measures a player's total contribution to their team compared to a replacement-level player. It can be interpreted as the number of additional wins a player brings to their team. For example, if a player's WAR is four, they would be expected to bring four additional wins to their team compared to an average player a team could sign to replace them. This metric combines multiple aspects of a player’s performance, including offense, defense, baserunning, and positional value, into a single metric. MLB front offices and fans alike use WAR to evaluate and compare player performance.
...
### OLS Comparison Plot
```{r ols-comp-plotz, echo=FALSE, fig.pos='H', out.width="85%", out.height="3.2in"}
knitr::include_graphics("Pictures/final_ols_comp_plot.png")
```

```{r ols-comp-ploti, echo=FALSE, fig.pos='H', out.width="85%", out.height="3.2in"}
...
```{r boost-comp-plotes, echo=FALSE, fig.align="center", out.width="90%"}
knitr::include_graphics("Pictures/LASSO_comp_plot.png")
```
```{r lasso-comp-plot, echo=FALSE, fig.pos='H', out.width="85%", out.height="3.2in"}
...
```{r boost-comp-plot, echo=FALSE, fig.pos='H', out.width="85%", out.height="3.2in"}
knitr::include_graphics("Pictures/boost_comp_plot.png")
```

```{r boost-comp-ploty, echo=FALSE, fig.pos='H', out.width="85%", out.height="3.2in"}
...
```{r war-examples, echo=FALSE, fig.pos='H', out.width="85%", out.height="3.2in"}
